...
#the dependent variable ("Default" in my case) should be the first column of the testset for the correct operation of the function down below
...
ANN_simulation <- function(formula, train_set, test_set, neurons, times, threshold = 0.01, stepmax = 100000, 
  ...
  for(i in 1:neurons) { 
    ...
    for(i in 1:total_simulation) {
      ...
      roundedresults$actual <- as.numeric(unlist(test[1]))
